...
## router/shared; patch only needed by RT-N and RT-AC
...
## others; specific version for RT needed
patch -i $FT_PATCHES_DIR/libfoo.pl.patch $FT_REPO_DIR/release/src/btools/libfoo.pl

### kernel built-in
## turn off warnings about unused functions/variables
patch -i $FT_PATCHES_DIR/Makefile_linux_RT-N.patch $FT_REPO_DIR/release/$RT_VERS/linux/linux-2.6/Makefile
...
## kernel-modules: et-driver; one patch can only applied to RT-N and RT-AC; one specific version for RT needed
patch -i $FT_PATCHES_DIR/etc.c_RT.patch $FT_REPO_DIR/release/$RT_VERS/et/sys/etc.c
patch -i $FT_PATCHES_DIR/etc_adm.c_RT-N.patch $FT_REPO_DIR/release/$RT_VERS/et/sys/etc_adm.c
#patch -i $FT_PATCHES_DIR/etc47xx.c_RT-N.patch $FT_REPO_DIR/release/$RT_VERS/et/sys/etc47xx.c
patch -i $FT_PATCHES_DIR/etcgmac.c_RT-N.patch $FT_REPO_DIR/release/$RT_VERS/et/sys/etcgmac.c

## compressed kernel 
patch -i $FT_PATCHES_DIR/bzip2_inflate.c_RT-N.patch $FT_REPO_DIR/release/$RT_VERS/shared/bzip2_inflate.c
## lzma-loader
